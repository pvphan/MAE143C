...
t1 = abs(c2a*c3-c1^2), t2 = c1 + c3, t2hat = c1 + c2a, 
Ts = 0.005; % seconds
...
numD1 = K1*PolyConv([1 P_G1(1)],[1 P_G1(2)]);  denD1 =PolyConv([1 pa], [1 0]);
numD1a= 10*K1*PolyConv([1 P_G1(1)],[1 P_G1(2)]);  denD1a=PolyConv([1 5*pa],[1 0]); % edit Gain and denD1
numD1b= K1*PolyConv([1 P_G1(1)],[1 P_G1(2)]);  denD1b=PolyConv([1 pa], [1 .05]); % edit denD1
...
% Problem #1a
figure(1); g.K=logspace(-1.5,1.5,400); g.axes=[-22 12 -16 16];
RLocus(numG1,denG1,numD1,denD1,g); % print -depsc figs/final_143b_13_1a.eps
% RLocus(numG1,denG1,numD1a,denD1a,g); % D1a
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	za = z1
	s_plus = -sigma + omega_d*i
	a = (s_plus + za)*(s_plus - z1); b = s_plus^2;
	K2_initial = real(a/b);
...
numD2a = 0.5*K2*[1 za]; denD2a =[1 z1];
...
RLocus(numG2,denG2,numD2,denD2,g); % print -depsc figs/final_143b_13_2a.eps
...
g.styley='b-';  ResponseTF(numH2,denH2,1,g)
