...
P_G1 = roots([1 a2 -a1 -a0]); Z_G1 = roots([k1 0]);
...
disp('Bode properties of D1(s):'); disp('')
	% [Gm, Pm, omega_p, omega_c] = margin(D1), disp('')
	omega_c = omega_n;

disp('Coefficients of D1(z):'), disp('')
...
%%%%%% ROOT LOCUS %%%%%%%%
	disp('')
	figure(1), g.K = logspace(-1.5, 1.5, 200); g.axes=[-(1.5)*omega_n (0.5)*omega_n -omega_n omega_n ];
...
%%%%%% BODE PLOT %%%%%%%
	...
	% BLUE: D1(z)
	g.style='b-'; [bz,az]=C2DTustin(numD1,denD1,h,omega_c);   Bode(bz,az,g,h)	
	% RED: G1(z)*D1(z)
